...
//  pig
//
//  Created by caden christesen on 11/15/22.
//
...
struct InstructionsView: View {
    var body: some View {
        ZStack {
            ...
            VStack {
                Image("Pig").resizable().frame(width: 150, height: 150)
                Text("How To 'Roll' With Pig").font(.title)
                VStack (alignment: .leading) {
                    ...
                    Text("If a player chooses to \"hold\", their turn total is added to their game score, and it becomes the next player's turn.")
                        .padding()
                    Text("A player wins the game when the game score becomes 100 or more on their  turn.") .padding()
                }
                Spacer()
            }
        }
